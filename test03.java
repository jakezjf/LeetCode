/**
 * Created by zhongjianfeng on 2017/5/6.
 */
public class GM {
    ...
    public static double getPredictiveValue(double[] originalArray, int num) {
        ...
        double k = (originalArray[0] - au) * (Math.exp(-a * (num - 1)) - Math.exp(-a * (num - 2)));
        for(int i = 0 ;i<5;i++){

            double temp = (originalArray[i] - k)
                    * (Math.exp(-a * (i - 1)) ) + k;
    ...
    public static void main(String[] args) {
        double[] originalArray = { 120.56 ,119.45 ,118.06 ,117.32 ,116.63 };
//        double[] originalArray = { 113.30 ,112.59 ,111.88 };
